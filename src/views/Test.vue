            ...
            el-row(:gutter="16")
              el-col(:span="24")
                el-form-item(label="Nombre completo")
                  el-input(v-model="meetForm.name")
              el-col(:span="12")
              ...
              el-col(:span="12")
                el-form-item(label="Edad")
                  el-input-number(v-model="meetForm.age", style="width: 100%")

          el-button(@click="user.exists = undefined; user.input = ''") Cambiar usuario
          el-button(
            :loading="loading",
            @click="user.exists ? login() : login()",
            type="success"
          ) Continuar
      ...
      if (ignore || step.value > 0) {
        if (step.value > 0 && selected.value.alternatives) {
          ...
          for (const item of answers) {
            ...
            helper.set(item.modalityId, old + (item.weight ?? item.score ?? 1));
          }
